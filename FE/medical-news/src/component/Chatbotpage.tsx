...
import { FiSend, FiPlus, FiMessageCircle, FiImage, FiX, FiAlertCircle, FiCheckCircle } from "react-icons/fi";
...
/** Thanh % độ tin cậy nhỏ gọn */
const ConfidenceBar = ({ label, score }: { label: string; score: number }) => {
  const pct    = Math.round(score * 100);
  const color  = pct >= 75 ? "#e53e3e" : pct >= 50 ? "#dd6b20" : "#d69e2e";
  return (
...
/** Card kết quả X-ray hiển thị trong bubble của bot */
const XrayResultCard = ({ result }: { result: XrayFinding }) => {
  if (result.no_finding) {
    return (
      <div style={{
        marginTop: 10, padding: "10px 14px", background: "#f0fff4",
        border: "1px solid #9ae6b4", borderRadius: 10,
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6, color: "#276749", fontWeight: 600 }}>
          {ico(FiCheckCircle, 15)} Không phát hiện bất thường
        </div>
        <p style={{ margin: "4px 0 0", fontSize: 12, color: "#4a5568" }}>
          AI không phát hiện dấu hiệu bệnh lý rõ ràng. Vẫn nên thăm khám định kỳ.
        </p>
      </div>
    );
  }
  ...
  return (
    <div style={{
      ...
      border: "1px solid #f6e05e", borderRadius: 10,
    }}>
      <div style={{ display: "flex", alignItems: "center", gap: 6, color: "#744210", fontWeight: 600, marginBottom: 10 }}>
        {ico(FiAlertCircle, 15)} Kết quả phân tích X-quang AI
      </div>
      {result.findings_vi.map((vi, i) => (
        <ConfidenceBar
          key={vi}
          label={vi}
          score={result.scores[result.findings[i]] ?? 0}
        />
      ))}
      <p style={{ margin: "8px 0 0", fontSize: 11, color: "#718096", fontStyle: "italic" }}>
        ⚠️ Kết quả AI chỉ mang tính hỗ trợ, không thay thế chẩn đoán của bác sĩ.
      </p>
    </div>
  );
};
...
const ChatBotPage = () => {
  ...
  const loadMessages = async (convId: string) => {
    try {
      ...
      setMessages((res.data.result || []).map((msg: any) => ({
        ...
        // xrayResult không lưu lại DB nên không restore
      })));
    } catch (err) { console.error(err); }
  };
  ...
  const handleNewChat = () => {
    ...
    setMessages([]);
    if (!isLoggedIn) sessionStorage.removeItem(SESSION_KEY);
      ...
      if (isLoggedIn) {
        ...
        if (!convId) {
          const res = await authApis().post(endpoint.chat_create, {
            title: currentText || "Phân tích X-quang",
          });
        ...
        const botResponse  = res.data.result?.botResponse  ?? res.data.answer  ?? "";
        const xrayResult   = res.data.result?.xrayResult   ?? res.data.xray_result ?? null;
        ...
        setTimeout(() => {
          setMessages(prev => [...prev, {
            text:      "Bạn cần đăng nhập để sử dụng đầy đủ tính năng hỏi đáp y tế và phân tích X-quang.",
            sender:    "bot",
            timestamp: new Date(),
          }]);
          setIsLoading(false);
      ...
      <main className={styles.chatMain}>
        ...
        <div className={styles.messageList}>
          {messages.length === 0 && (
            <div className={styles.welcome}>
              ...
              <p className={styles.welcomeDesc}>
                Hỏi tôi về sức khỏe hoặc gửi ảnh X-quang ngực để phân tích bằng AI.
              </p>
          ...
          {messages.map((msg, i) => (
            <div
              key={i}
              className={`${styles.msgRow} ${msg.sender === "user" ? styles.msgRowUser : styles.msgRowBot}`}
            >
              {msg.sender === "bot" && (
              ...
              <div className={`${styles.bubble} ${msg.sender === "user" ? styles.bubbleUser : styles.bubbleBot}`}>
                {/* Ảnh người dùng gửi */}
                {msg.imageUrl && (
                  <img src={msg.imageUrl} alt="X-ray" className={styles.msgImg} />
                )}

                {/* Text */}
                {msg.text && <p className={styles.bubbleText}>{msg.text}</p>}

                {/* X-ray result card (chỉ trong bubble của bot) */}
                {msg.sender === "bot" && msg.xrayResult && (
                  <XrayResultCard result={msg.xrayResult} />
                )}
        ...
        <div className={styles.inputArea}>
          ...
          <div className={styles.inputRow}>
            ...
            <input
              ...
              type="text"
              placeholder={selectedFile ? "Thêm mô tả cho ảnh X-quang (tùy chọn)…" : "Nhập câu hỏi y tế…"}
              value={inputMessage}
          ...
          <p className={styles.disclaimer}>
            ...
            Kết quả X-quang AI cần được bác sĩ xác nhận.
          </p>
        </div>
      </main>
    </div>
